...
| Versión | Hito |
|---|---|
| 0.7.17 | **Obstáculos del entorno + multi-fabricante (Fase I núcleo) + estilo ISO en planos**. (A) **Adaptación a obstáculos** (convención completa en `OBSTACLES.md`): collection `obstaculos` con meshes `Terrain_*` (raycast vertical por poste → husillo individual `ref_z - terrain_z`, warning si jack > 80 cm), `Wall_*` (split/clipping de plataformas, tubos, barandillas y rodapiés a ambos lados del plano; trampillas omitidas si la pared cae sobre el lid; postes dentro de pared → error fatal) y `Volume_*` (vanos cuyo centro cae dentro se skipan enteros; parciales → clipping). Multi-tramo automático cuando el salto Z entre empties consecutivos supera 1 m (`_TRAMO_SPLIT_DZ_M`): tramos independientes con prefijo `T0_`, `T1_`… (el BOM strip-ea el prefijo al clasificar). Sub-panel "Obstáculos" + aviso explícito de que el FEM no está validado con piezas custom. Casos T1-T6/W1-W5/V1/C1-C2 documentados. (B) **Multi-fabricante Fase I**: nuevo `calc/catalogs.py` (puro Python) como fuente única dimensional — Layher Allround, PERI UP Rosett Flex (largueros UH Plus 0,25–3,0 m en retícula de 25 cm, verticales UVR 0,5–4,0 m, decks UDG 25/37,5 cm), ULMA BRIO (brazos 0,35–3,0 m, pies 1–4 m) y Doka Ringlock S (largueros 0,39–3,07 m, verticales 0,5–3,0 m), con pesos por pieza verificados contra los catálogos oficiales (PERI/ULMA). Addon: `_ensure_manufacturer_catalogs()` (lazy, en register y generate) vuelca calc.catalogs en `_BAY/_POLE_LENGTH_CATALOGS`; 6 presets nuevos (PERI_UP_75/100, ULMA_BRIO_70/102, DOKA_73/109) que fijan profundidad/bandejas/ambos catálogos; enums ampliados añadiendo al final (no rompe escenas guardadas); selector de catálogos pasa de fila expandida a dropdown (6 ítems comprimían las etiquetas). CAD: `generate_cad_sheets(bay_steps_m=…)` snapea las cotas al catálogo activo en vez de asumir Layher. 26 tests nuevos (`test_catalogs.py`). (C) **2 fixes en `_bay_lengths_for_segment`** descubiertos probando PERI en vivo vía MCP: el DP discretizaba a cm y convertía el larguero de 37,5 cm en una pieza inexistente de 38 cm (ahora discretiza a mm); las locations float32 de Blender (6.3 → 6.30000019) hacían que un sobrante nominal de 0,05 superara `max_leftover` por µm y creara un vano de compensación de 5 cm inmontable (tolerancia +1 mm). (D) **Estilo normativo en planos CAD** (`cad_plan.py`): anchos de línea cuantizados a UNE-EN ISO 128-20 (0,18/0,25/0,35/0,50 mm), fuentes con mínimo legible ISO 129-1 (2,5 mm), paleta de 4 colores estructurales + marcadores funcionales. (E) **Build**: el zip incluye `tutorial_guide.py` e `i18n/` (antes el tutorial guiado no cargaba al instalar el zip oficial). **460/460 PASSED** + smoke headless 3 fabricantes + prueba en vivo (ULMA/DOKA encajan 6,3 m exacto con piezas reales; PERI deja compensación de 0,3 m, correcta porque 6,3 no es alcanzable en su retícula). |
| 0.7.16 | Release de transición: automatización de release (workflow CI `release.yml`) + bump combinado con features WIP, sin notas detalladas (ver commit 18928cc). |
